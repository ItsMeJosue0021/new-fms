<x-admin>
    <div>
        ...
        <div class="p-5 rounded-lg bg-white ">
            <form method="POST" enctype="multipart/form-data" action="{{ route('hearses.update', $hearse->id) }}" class="flex flex-col w-full">
                @method('PUT')
                @csrf
                <div class="flex space-x-4">
                    <div class="flex flex-col space-y-2 w-full">
                        <label for="name" class="text-sm font-semibold">Name</label>
                        <input id="name" name="name" type="text" placeholder="Product name" class="text-sm rounded-md border border-gray-200" value="{{ $hearse->name }}"/>
                        @error('name')
                ...
                <div class="py-4">
                    ...
                    <div class="grid grid-cols-5 gap-4">
                        @forelse ($hearse->hearseImages as $image)
                            <div class="relative h-56 max-w-full group cursor-pointer border border-gray-200 rounded-md">
                                ...
                                <div class="group-hover:bg-gray-500 group-hover:bg-opacity-50 top-0 left-0 rounded-md absolute w-full h-full flex items-center justify-center">
                                    <a href="{{ route('hearses.delete-image', $image->id) }}" class="hidden group-hover:block text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:ring-red-300 font-medium rounded-md text-sm px-5 py-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">Delete</a>
                                </div>
                            </div>
                        @empty
