<div class="flex flex-col space-y-5">
    ...
    <div class="w-full flex flex-col">
        ...
        class="focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            <option value="" selected disabled>Select a Cause of Death</option>
                ...
                <option value="{{ $cause->name }}" {{ (old('death_cause') ?? ($service->deceased->deathDetail->death_cause ?? '')) == $cause->name ? 'selected' : '' }}>{{ $cause->name }}</option>
                @endforeach
        ...
        style="display: none;"
        value="{{ old('other_death_cause') ?? ($service->deceased->deathDetail->death_cause ?? '') }}">
        @error('death_cause')
    ...
    <div class="w-full flex flex-col space-y-1">
        ...
        class="focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        value="{{ old('death_place') ?? ($service->deceased->deathDetail->death_place ?? '') }}">
        @error('death_place')
            <span class="text-xs text-red-500 pl-2">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="w-full flex space-x-4 items-start">
        <div class="w-full flex flex-col space-y-1">
            ...
            class="focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            value="{{ old('death_time') ?? ($service->deceased->deathDetail->death_time ?? '') }}">
            @error('death_time')
                <span class="text-xs text-red-500 pl-2">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="w-full flex flex-col space-y-1">
            ...
            <input type="date" name="death_date" placeholder="Date of death" max="{{ date('Y-m-d') }}"
            class="focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            value="{{ old('death_date') ?? ($service->deceased->deathDetail->death_date ?? '') }}">
            @error('death_date')
                <span class="text-xs text-red-500 pl-2">{{ $message }}</span>
            @enderror
        </div>
