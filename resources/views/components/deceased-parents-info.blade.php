<div class="flex flex-col space-y-5">
    ...
    <div class="w-full flex space-x-4 items-start">
        <div class="w-full flex flex-col">
            ...
            <option value="" {{ (old('occupation') ?? ($service->deceased->occupation ?? '')) == '' ? 'selected' : '' }} disabled>Choose an Occupation</option>
                @foreach ($jobs as $job)
                    <option value="{{ $job->name }}" {{ (old('occupation') ?? ($service->deceased->occupation ?? '')) == $job->name ? 'selected' : '' }}>{{ $job->name }}</option>
                @endforeach
        ...
        <div class="w-full flex flex-col">
            ...
            <select name="religion" id="religion" class="focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="" {{ (old('religion') ?? ($service->deceased->religion ?? '')) == '' ? 'selected' : '' }} disabled>Choose Religion</option>
                @foreach ($religions as $religion)
                    <option value="{{ $religion->name }}" {{ (old('religion') ?? ($service->deceased->religion ?? '')) == $religion->name ? 'selected' : '' }}>{{ $religion->name }}</option>
                @endforeach
                <option value="Other" >Other (Specify Below)</option>
            </select>
        ...
        <div class="w-full flex flex-col">
            ...
            class="focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="" {{ (old('citizenship') ?? ($service->deceased->citizenship ?? '')) == '' ? 'selected' : '' }} disabled>Choose a Citizenship</option>
                <option value="Filipino" {{ (old('citizenship') ?? ($service->deceased->citizenship ?? '')) == 'Filipino' ? 'selected' : '' }}>Filipino</option>
                <option value="Dual Citizen" {{ (old('citizenship') ?? ($service->deceased->citizenship ?? '')) == 'Dual Citizen' ? 'selected' : '' }}>Dual Citizen</option>
                <option value="Naturalized Citizen" {{ (old('citizenship') ?? ($service->deceased->citizenship ?? '')) == 'Naturalized Citizen' ? 'selected' : '' }}>Naturalized Citizen</option>
                <option value="Former Filipino Citizen" {{ (old('citizenship') ?? ($service->deceased->citizenship ?? '')) == 'Former Filipino Citizen' ? 'selected' : '' }}>Former Filipino Citizen</option>
                <option value="Other">Other (Specify Below)</option>
            </select>
        ...
        <div class="w-full flex flex-col space-y-1">
            ...
            class="mt-2 focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="" {{ (old('civil_status') ?? ($service->deceased->civil_status ?? '')) == '' ? 'selected' : '' }} disabled>Choose a Civil Status</option>
                <option value="Married" {{ (old('civil_status') ?? ($service->deceased->civil_status ?? '')) == 'Married' ? 'selected' : '' }}>Married</option>
                <option value="Single" {{ (old('civil_status') ?? ($service->deceased->civil_status ?? '')) == 'Single' ? 'selected' : '' }}>Single</option>
                <option value="Separated" {{ (old('civil_status') ?? ($service->deceased->civil_status ?? '')) == 'Separated' ? 'selected' : '' }}>Separated</option>
                <option value="Widowed" {{ (old('civil_status') ?? ($service->deceased->civil_status ?? '')) == 'Widowed' ? 'selected' : '' }}>Widowed</option>
            </select>
            @error('civil_status')
                <span class="text-xs text-red-500 pl-2">{{ $message }}</span>
            @enderror
    ...
    <div class="w-full flex flex-col space-y-1">
        ...
        class="mt-2 focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        value="{{ old('birth_place') ?? ($service->deceased->birth_place ?? '') }}">
        @error('birth_place')
            <span class="text-xs text-red-500 pl-2">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="w-full flex space-x-4 items-start">
        <div class="w-full flex flex-col space-y-1">
            ...
            class="mt-2 focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            value="{{ old('fathers_name') ?? ($service->deceased->fathers_name ?? '') }}">
            @error('fathers_name')
                <span class="text-xs text-red-500 pl-2">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="w-full flex flex-col space-y-1">
            ...
            class="mt-2 focus:bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            value="{{ old('mother_maiden_name') ?? ($service->deceased->mother_maiden_name ?? '') }}">
