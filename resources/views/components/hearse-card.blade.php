<div class="p-4 w-full md:w-1/3">
    <div class="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
        ...
        <div id="animation-carousel" class="relative w-full" data-carousel="static">
            <!-- Carousel wrapper -->
            <div class="relative lg:h-48 sm:h-36 overflow-hidden z-0">
                @if (count($hearse->hearseImages) > 0)
                @foreach ($hearse->hearseImages as $image)
                <div class="hidden duration-200 ease-linear" data-carousel-item>
                    <img src="{{ asset('storage/' . $image->image) }}" alt="{{ $hearse->name }}" class="zoomable-image absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 cursor-pointer">
                </div>
        ...
        <div class="p-4">
            <h1 class="title-font text-lg font-medium text-gray-900 ">{{ $hearse->name }}</h1>
            <p class="leading-6 mb-3 text-sm">{{ $hearse->description }}</p>

            <div class="flex items-center justify-between z-50">
                <a href="{{ route('services.hearses-select', ['serviceId' => $service->id, 'hearse' => $hearse->id]) }}" class="text-white text-sm px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 flex items-center">Select and proceed
