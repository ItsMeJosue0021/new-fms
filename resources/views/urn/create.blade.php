<x-admin>
    <div>
        ...
        <div class="flex mb-2 px-2">
            <h1 class="text-lg bg-medium">Add New Urn</h1>
        </div>
        <div class="p-5 rounded-lg bg-white ">
            <form method="POST" action="{{ route('urns.store') }}" class="flex flex-col w-full" enctype="multipart/form-data">
                @csrf
                ...
                <div class="flex space-x-4 mt-3">
                    ...
                    <div class="flex flex-col space-y-2 w-full">
                        ...
                        <input id="stock" name="stock" type="number" placeholder="Stock" class="text-sm rounded-md border border-gray-200"/>
                        @error('stock')
                            <p class="text-red-500 text-xs">{{$message}}</p>
                        @enderror
                    </div>
                    <div class="flex flex-col space-y-2 w-full">
                        ...
                        <input id="price" name="price" type="number" placeholder="Price" class="text-sm rounded-md border border-gray-200"/>
                        @error('price')
                            <p class="text-red-500 text-xs">{{$message}}</p>
                        @enderror
                    </div>
                    <div class="flex flex-col space-y-2 w-full">
                        ...
                        <input id="water" name="water" type="number" placeholder="0" class="text-sm rounded-md border border-gray-200 w-full"/>
                        @error('water')
                            <p class="text-red-500 text-xs">{{$message}}</p>
                        @enderror
                    </div>
